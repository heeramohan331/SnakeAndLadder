    ...
    public void rolldice(MouseEvent event) throws IOException {
        ...
        if (randomnumber != 6) {
            if (turn == 1) {
                ...
                turnname.setFill(Paint.valueOf("red"));
                turn = 2;
            } else {
                turnname.setText("PLAYER 1 TURN");
                turnname.setFill(Paint.valueOf("blue"));
                turn = 1;
            }
        }
        ...
        void getSnakeLadderCoordinates(){
            ...
            snakeLadderCoordinateChanges.put(new Pair<>(350.0,-400.0),new Pair<>(200.0, -150.0));
        ...
        void checkWin(Double X, Double Y) throws IOException
        {
            if(X==50 && Y==-450){
                ...
                if(turn==1){
                    resultAlert.setContentText("Congrats!!! Player 1 Wins");
                    resultAlert.showAndWait();
                    StartGame start=new StartGame();
                    HelloApplication.root.getChildren().setAll(start.root);
                }
                else{
                    resultAlert.setContentText("Congrats!!! Player 2 Wins");
                    resultAlert.showAndWait();
                    StartGame start=new StartGame();
                    HelloApplication.root.getChildren().setAll(start.root);
                }
